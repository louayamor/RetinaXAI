...
class MLServiceClient:
    ...
    async def predict(self, request: MLPredictRequest) -> MLPredictResponse:
        ...
        logger.info(f"[MLCLIENT] model={request.model_name} version={request.model_version}")
        logger.info(f"[MLCLIENT] left_scan_path={request.left_scan_path} exists={Path(request.left_scan_path).exists()}")
        logger.info(f"[MLCLIENT] right_scan_path={request.right_scan_path} exists={Path(request.right_scan_path).exists()}")
        logger.info(f"[MLCLIENT] patient age={request.patient_age} gender={request.patient_gender}")
        ...
        try:
            ...
            logger.error(f"[MLCLIENT] Timeout connecting to ML service at {self.base_url}")
            raise ServiceUnavailableException("ml-service")
        except httpx.ConnectError as e:
            logger.error(f"[MLCLIENT] Connection error to ML service at {self.base_url}: {e}")
            raise ServiceUnavailableException("ml-service")
        except httpx.HTTPStatusError as e:
            logger.error(f"[MLCLIENT] HTTP error {e.response.status_code}: {e.response.text}")
            raise
