...
def generate_reports(settings: Settings = Depends(get_settings)):
    ...
    imaging_train = Path(
        os.environ.get(
            "IMAGING_TRAIN_CSV", "artifacts/data/processed/imaging/train.csv"
        )
    )
    imaging_test = Path(
        os.environ.get("IMAGING_TEST_CSV", "artifacts/data/processed/imaging/test.csv")
    )
    imaging_samaya = Path(
        os.environ.get(
            "IMAGING_SAMAYA_CSV", "artifacts/data/processed/imaging/samaya.csv"
        )
    )
    clinical_train = Path(
        os.environ.get(
            "CLINICAL_TRAIN_CSV", "artifacts/data/processed/clinical/train.csv"
        )
    )
    clinical_test = Path(
        os.environ.get(
            "CLINICAL_TEST_CSV", "artifacts/data/processed/clinical/test.csv"
        )
    )

    missing = [
        str(p)
        for p in [imaging_train, imaging_test, clinical_train, clinical_test]
        if not p.exists()
    ]
    if missing:
        raise HTTPException(
    ...
    try:
        generator.imaging_data_drift(
            imaging_train, imaging_test, reports_dir / "imaging_drift_report.html"
        )
    ...
    try:
        generator.clinical_data_drift(
            clinical_train, clinical_test, reports_dir / "clinical_drift_report.html"
        )
    ...
    if imaging_samaya.exists():
        try:
            generator.domain_shift_report(
                imaging_test, imaging_samaya, reports_dir / "domain_shift_report.html"
            )
