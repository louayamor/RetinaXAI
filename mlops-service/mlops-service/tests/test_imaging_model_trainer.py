...
def test_imaging_model_trainer_logs_model_with_cpu_example(monkeypatch, tmp_path):
    ...
    trainer.transformation_config = type(
        "T",
        (),
        {"train_csv": tmp_path / "train.csv", "test_csv": tmp_path / "test.csv"},
    )()  # type: ignore[assignment]
    ...
    monkeypatch.setattr(
        mt.ImagingModelTrainer, "_build_transforms", lambda self: (None, None)
    )
    monkeypatch.setattr(mt.ImagingModelTrainer, "_build_sampler", lambda self, *_: None)
    monkeypatch.setattr(
        mt.ImagingModelTrainer, "_build_model", lambda self: DummyModel()
    )
    ...
    monkeypatch.setattr(
        mt.BEST_VAL_ACCURACY, "labels", lambda **kwargs: mt.BEST_VAL_ACCURACY
    )
    monkeypatch.setattr(mt.BEST_VAL_ACCURACY, "set", lambda *_: None)
    monkeypatch.setattr(
        mt.EPOCH_TRAIN_LOSS, "labels", lambda **kwargs: mt.EPOCH_TRAIN_LOSS
    )
